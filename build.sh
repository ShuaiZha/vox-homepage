...
#   go build -o "vox-homepage" --ldflags="-X vox-homepage/global.RUNCODE=release" main.go
# }
...
_build() {
  ...
  if [ -n "$VERSION" ]; then
    outPath="vox-homepage_${VERSION}_${os}_${arch}"
  elif [ -n "$LATEST_TAG" ]; then
    outPath="vox-homepage_${LATEST_TAG}_${os}_${arch}"
  else
    outPath="vox-homepage_${COMMIT_SHA}_${os}_${arch}"
  fi
  outname="${pathRelease}/${outPath}/vox-homepage"
  go build -o "${outname}" --ldflags="-X vox-homepage/global.RUNCODE=release" main.go
  ...
  if [ "$os" = "windows" ]; then
    mv $outname $outPath/vox-homepage.exe
    zip -r "${pathRelease}/${outPath}.zip" $outPath
  else
    mv $outname $outPath/vox-homepage
...
buildReleaseLinuxMusl() {
  cd $REPO/service
  ldflags="-X vox-homepage/global.RUNCODE=release"
  pathRelease=$REPO/release
  ...
  for i in "${!OS_ARCHES[@]}"; do
    ...
    if [ -n "$VERSION" ]; then
      outPath="vox-homepage_${VERSION}_${GOOS}_musl_${GOARCH}"
    elif [ -n "$LATEST_TAG" ]; then
      outPath="vox-homepage_${LATEST_TAG}_${GOOS}_musl_${GOARCH}"
    else
      outPath="vox-homepage_${COMMIT_SHA}_${GOOS}_musl_${GOARCH}"
    fi
